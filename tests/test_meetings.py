...
class TestCreateMeeting:
    async def test_creates_meeting_successfully(self, client, db):
        ...
        response = await client.post(
            "/meetings",
            params={"team_id": 1},
            json={"title": "Планёрка", "start_time": start, "end_time": end, "team_id": 1},
        )
    ...
    async def test_created_meeting_has_creator_id(self, client, db):
        ...
        response = await client.post(
            "/meetings",
            params={"team_id": 1},
            json={"title": "Ретро", "start_time": start, "end_time": end, "team_id": 1},
        )
        assert response.status_code == 201
...
class TestGetMeetings:
    async def test_returns_empty_list_for_team_without_meetings(self, client, db):
        ...
        response = await client.get("/meetings/team/1", params={"team_id": 1})
    ...
    async def test_does_not_return_meetings_of_other_team(self, client, db):
        await create_user(db, uid=1)
        await create_team(db, owner_id=1, tid=1, name="Team 1")
        await create_team(db, owner_id=1, tid=2, name="Team 2")
...
class TestDeleteMeeting:
    async def test_deletes_meeting_successfully(self, client, db):
        ...
        response = await client.delete(
            f"/meetings/{meeting.id}", params={"team_id": 1}
        )

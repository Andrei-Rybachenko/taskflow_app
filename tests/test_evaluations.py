...
class TestAddEvaluation:
    async def test_adds_score_for_task(self, client, db):
        await create_user(db, uid=1, username="manager", email="mgr@test.com")
        ...
        response = await client.post(
            ...
            params={"team_id": 1},
            json={"score": 5, "comment": "Отличная работа", "employee_id": 2},
        )
    ...
    async def test_returns_404_for_nonexistent_task(self, client, db):
        ...
        response = await client.post(
            ...
            params={"team_id": 1},
            json={"score": 3, "comment": "ok", "employee_id": 1},
        )
    ...
    async def test_returns_400_if_task_not_assigned(self, client, db):
        ...
        response = await client.post(
            f"/evaluations/task/{task.id}",
            params={"team_id": 1},
            json={"score": 4, "comment": "Хорошо", "employee_id": 1},
        )
        assert response.status_code == 400

    async def test_cannot_add_duplicate_evaluation(self, client, db):
        await create_user(db, uid=1)
        await create_user(db, uid=2, username="exec", email="exec@test.com")
        await create_team(db, owner_id=1, tid=1)
        await create_membership(db, user_id=1, team_id=1, role=Role.MANAGER)
        task = await create_task(
            db,
        ...
        eval_orm = EvaluationORM(
            task_id=task.id, reviewer_id=1, employee_id=2, score=4, comment="First"
        )
        db.add(eval_orm)
        await db.commit()
        ...
        response = await client.post(
            f"/evaluations/task/{task.id}",
            params={"team_id": 1},
            json={"score": 5, "comment": "Second", "employee_id": 2},
        )
        assert response.status_code == 400
...
class TestGetEvaluation:
    async def test_returns_evaluation_for_task(self, client, db):
        ...
        eval_orm = EvaluationORM(
            task_id=task.id, reviewer_id=1, employee_id=2, score=5, comment="Отлично"
        )
        db.add(eval_orm)
        await db.commit()
